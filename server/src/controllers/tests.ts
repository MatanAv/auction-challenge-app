...
import { BONUS_MULTIPLIER, NON_GROUP_MAX_TIMES_USED, GROUP_MAX_TIMES_USED, TEST_NON_GROUP_QUESTIONS_AMOUNT } from '@/constants';
...
const getTestSummary = async (worker_id: string, answers: IUserTestAnswer[], duration: number): Promise<ResponseFormat> => {
    try {
...
const handleTimeout = async (worker_id: string, isTraining: boolean, results: IUserTraining | IUserTest): Promise<ResponseFormat> => {
    try {
        return isTraining
