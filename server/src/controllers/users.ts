...
const createUser = async (worker_id: string, full_name: string, req: Request): Promise<ResponseFormat> => {
    try {
        const user = await User.create({ worker_id, full_name });
...
const updateUserInstructions = async (worker_id: string, user_instructions: IUserInstructions): Promise<ResponseFormat> => {
